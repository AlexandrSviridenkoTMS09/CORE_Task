...
public class Main {
    ...
    private static void sortPeople() {
        List<People> sortedPeople = persons.stream()
                .sorted(Comparator.comparing(People::getGender).thenComparing(People::getAge))
    ...
    private static void getAbleToWorkMen() {
        long ableToWorkMen = persons.stream()
                .filter(person -> person.getAge() >= 18 && ((person.getGender() == Gender.MAN && person.getAge() <= 60) || (person.getGender() == Gender.WOMAN && person.getAge() <= 55)))
                .count();
    ...
    private static void getAverageManAge() {
        long averageManAge = (int) persons.stream()
                .filter(person -> person.getGender() == Gender.MAN).mapToInt(People::getAge)
                .average()
                .getAsDouble();
    ...
    private static void findMilitary() {
        List<People> military = persons.stream()
                .filter(person -> person.getGender() == Gender.MAN && person.getAge() >= 18 && person.getAge() <= 27)
                .collect(Collectors.toList());
    ...
    private static void initPeople() {
        persons.add(new People("Basil", 16, Gender.MAN));
        persons.add(new People("Peter", 23, Gender.MAN));
        persons.add(new People("Helen", 42, Gender.WOMAN));
        persons.add(new People("Ivan", 69, Gender.MAN));
    }
